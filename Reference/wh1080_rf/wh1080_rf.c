...
uint16_t cmd_freq	= CMD_FREQ|0x620; // 433.92 MHz
	...
	uint16_t cmd_rcon = (CMD_RCON|RX_EN|VDI_DRSSI|LNA_0|RSSI_79);
...
// might need to look at this!!
	...
	*(gpio.addr + 2) = (*(gpio.addr + 2) & 0xff1fffff)|(0x001 << 6);
	...
	// *(gpio.addr + 2) = (*(gpio.addr + 2) & 0xfffffe3f) | (0x001 << 6);



	#ifdef RFM01
		printf("Initialising RFM01 on %08x\n", gpio.addr);
	#endif
	#ifdef RFM12B
	...
	*(gpio.addr + (0x1c >> 2)) = 1 << 22;
	...
	do {
		...
		if(rssi != oldrssi) {
			...
			printf("storing bit pulse duration \n");
			if(rssi == 0) {
				rssitime_buf[count] = rssitime - oldrssitime;
				if(++count == 500)
				count = 499;
			}
			printf("RSSI TIME:%d %d %d\n",count,rssitime_buf[count],rssi);
			oldrssi = rssi;
		...
		int packet_offset = 0;
		now = TIMER_ARM_COUNT;
		// printf("timeout %d, now %d, oldrssitime %d\n",timeout,now,oldrssitime);
		if(!timeout && (now - oldrssitime) > 5000) { // && count > 0

			uint8_t sig_matched = 0;
			printf("got data packet count %d\n", count);
			if(count > 60) { // then maybe something at least interesting
				printf("Count greater than 60 \n");
				// Look for device_id
				int idx;
				...
				if((count == 88) && sig_matched) { // then probably a data packet
					...
					// LED on
					// *(gpio.addr + (0x1c >> 2)) = 1 << 22;
					printf(" packet is 88 and signal matched, locking frequency");
			...
			if(count > 40) {
				printf("Got enough bits to dump stats, count%d",count);
		...
		// commented out to see if it is a speed issue
